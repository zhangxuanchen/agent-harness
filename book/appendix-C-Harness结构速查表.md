...
| # | 层 | 检查项 | 不做的代价 | 验证方法 | 章节 |
|---|---|--------|-----------|---------|------|
| G1 | G | 输入检查点：L1 正则过滤覆盖 OWASP Top 10 注入模式 | 提示注入导致数据泄露/越权 | 用 AgentDojo 100 条注入用例测试，拦截率 ≥ 95% | Ch10 KP 4.2.1 |
| G2 | G | 工具调用检查点：危险操作白名单 + 人工审批 | Agent 执行 `DROP TABLE`/`rm -rf` | grep 所有 @Tool，危险操作必须有 `requiresApproval=true` | Ch10 KP 4.2.2 |
| G3 | G | 输出检查点：PII 脱敏（正则 + 掩码） | 输出用户密码/身份证号 | 注入 50 条 PII 测试用例，泄露率 = 0 | Ch10 KP 4.2.1 |
| E1 | E | 沙箱隔离：非 root + cap-drop=ALL + seccomp | Agent 破坏宿主机文件系统 | `docker inspect` 验证容器参数 | Ch4 §4.2 |
| E2 | E | 资源配额：CPU/内存/磁盘/超时 | 死循环脚本打满节点 | 模拟死循环，30s 内被终止 | Ch4 KP 4.3.1 |
| E3 | E | 文件系统隔离：不与宿主机共享写目录 | Agent 覆盖生产配置文件 | 沙箱内尝试写 `/etc/`，应失败 | Ch4 KP 4.3.1 |
| L1 | L | 步数硬上限（默认 ≤30） | $47K 循环事故重演 | 配置检查 + 压测触发上限 | Ch7 KP 5.1.2 |
| L2 | L | per-session 成本上限 + 熔断 | 单用户无限循环拖垮组织预算 | 模拟超限，应自动终止 | Ch17 KP 4.3.1 |
| L3 | L | 连续重复检测（N=3 强制跳出） | Agent 在空结果上死循环 | 模拟连续 3 次相同 tool_call | Ch7 KP 5.1.2 |
| V1 | V | 100+ 任务回归评估集 | 改一处坏三处，上线即故障 | 评估集故障率 ≤ 10% | Ch9 KP 5.1.2 |
| V2 | V | CI 门禁：故障率 > 10% 阻断部署 | 带病上线 | 触发一次回归，验证门禁生效 | Ch16 KP 4.1.1 |
| O1 | O | 成本归因：五标签（user/session/task/model/tool） | 月底账单看不清钱花在哪 | 查询近 24h 成本，能按标签 GROUP BY | Ch8 KP 4.4.1 |
| O2 | O | 事件日志：JSONL append-only + traceId | 出事后无法回放推理过程 | 抽查一条 trace，能完整还原 Agent 决策链 | Ch8 KP 4.5.1 |
| T1 | T | 工具描述五要素（名称/参数/返回值/错误/示例） | 55% → 92% 准确率的差距 | 逐个工具检查 description 字段 | Ch5 KP 5.6.1 |
| T2 | T | 结构化错误协议（errorCode/suggestion/retryable） | Agent 无法程序化自愈 | 触发一次工具错误，验证返回 JSON | Ch5 KP 5.5.4 |

### P1 门禁项（≥80% 必须通过）

| # | 层 | 检查项 | 不做的代价 | 验证方法 | 章节 |
|---|---|--------|-----------|---------|------|
| G4 | G | L2 分类器检测（<50ms，覆盖 15% 高级攻击） | 正则漏掉变体注入 | 用 AgentDojo 高级用例测试 | Ch10 KP 4.2.2 |
| G5 | G | YAML 声明式策略 + Git 版控 | 策略散落代码，不可审计 | 检查策略文件是否在 Git 仓库 | Ch10 KP 4.3.1 |
| G6 | G | WORM 审计日志 + 决策链追溯 | 事故后无法归责 | 抽查一条决策，能追溯到 6 层归因 | Ch10 §10.5 |
| E4 | E | 沙箱预热池（消除冷启动） | 高并发时 2-5s 延迟 | 压测 P95 启动延迟 < 100ms | Ch4 KP 4.4.1 |
| E5 | E | 网络白名单（只允许清单内域名） | Agent 访问不该访问的服务 | 沙箱内 curl 非白名单域名，应失败 | Ch4 KP 4.3.2 |
| L4 | L | 检查点/恢复机制（崩溃后可续跑） | 跑 20 步崩溃从头开始 | 杀掉 Agent 进程，验证可从 checkpoint 恢复 | Ch7 KP 5.5.2 |
| L5 | L | 编排模式选型有书面记录 | 默认 ReAct 导致成本失控 | 检查选型决策树文档 | Ch7 §7.6 |
| V3 | V | LLM-as-Judge 三招偏差治理 + Pearson r > 0.8 | 评分不可靠，回归误判 | 用标注集验证 Judge 一致性 | Ch9 §9.3 |
| V4 | V | A/B 测试 N ≥ 100 + p < 0.05 + Cohen's d > 0.2 | 小样本误判显著差异 | 检查最近一次 A/B 测试报告 | Ch9 §9.5 |
| O3 | O | P99 成本异常检测 + 告警 | 成本异常到月底才发现 | 注入异常成本，验证告警在 1h 内触发 | Ch8 KP 4.4.2 |
| O4 | O | 三层分级存储（热 7d / 温 30d / 冷归档） | 日志存储成本失控 | 检查存储策略配置 | Ch8 §8.6 |
| C1 | C | 上下文压缩机制（触发阈值 + 压缩策略） | 长对话 token 溢出 | 模拟 50 轮对话，验证压缩触发 | Ch6 KP 4.4.1 |
| C2 | C | KV-cache 稳定前缀（system+tools 不含动态内容） | 缓存命中率 0，成本翻倍 | 检查 system prompt 是否含时间戳/sessionId | Ch6 §6.3 |
| T3 | T | 工具数 >25 时启用 Semantic Router Top-K 预选 | 50+ 工具准确率崩塌到 0-20% | 检查路由配置 + 压测准确率 | Ch5 KP 5.4.2 |
...
| # | 层 | 检查项 | 价值 | 章节 |
|---|---|--------|------|------|
| G7 | G | L3 LLM 审查（<500ms，覆盖 5% 复杂攻击） | 拦截正则和分类器漏掉的语义攻击 | Ch10 KP 4.2.2 |
| E6 | E | Firecracker microVM（125ms 冷启动） | 多租户高安全场景的最强隔离 | Ch4 §4.5 |
| L6 | L | PipeReAct 混合编排（Pipeline 骨架 + ReAct 肌肉） | 成本与灵活性的最优平衡 | Ch7 KP 5.7.1 |
| V5 | V | 影子流量验证（5% 流量走新版本） | 全量部署前的最后防线 | Ch9 KP 5.5.1 |
| O5 | O | Guides vs Sensors 二分框架 | 前置引导 + 后置感知双保险 | Ch8 KP 4.2.1 |
| C3 | C | 三层记忆架构（工作/情景/语义） | 跨会话记忆 + 按需检索零浪费 | Ch6 §6.2 |
| C4 | C | 目标锚定 + 漂移检测（cosine < 0.6 告警） | 防止 Agent 偏离原始目标 | Ch6 KP 4.4.2 |
...
| E-4 | 资源配额（CPU/内存/磁盘） | P0 | 死循环打满节点 | `docker stats` 验证限制 | CPU 1核/内存 512MB/磁盘 1GB | Ch4 KP 4.3.1 |
| E-5 | 全局任务超时 + 单步工具超时 | P0 | 单步卡死拖垮整个任务 | 配置检查 + 压测 | 任务 30s / 工具 10s | Ch4 KP 4.3.1 |
| E-6 | 文件系统隔离（不共享 /tmp） | P0 | 覆盖生产配置 | 沙箱内写宿主机路径应失败 | 共享目录 = ∅ | Ch4 KP 4.3.1 |
| E-7 | 快照与回滚（OverlayFS） | P1 | 文件污染后无法恢复 | 触发回滚，验证毫秒级恢复 | 回滚 < 100ms | Ch4 KP 4.3.3 |
| E-8 | 沙箱预热池 | P1 | 高并发冷启动 2-5s | 压测 P95 启动延迟 | P95 < 100ms | Ch4 KP 4.4.1 |
| E-9 | 网络白名单代理 | P1 | Agent 访问不该访问的服务 | curl 非白名单域名应失败 | 拦截率 100% | Ch4 KP 4.3.2 |
| E-10 | 三层文件系统（Overlay+Composite+Projection） | P2 | 多用户场景存储成本高 | 检查文件系统架构配置 | 多用户场景启用 | Ch4 §4.7 |

### C.4.2 T 层 · 工具接口与协议

| # | 检查内容 | 优先级 | 不做的代价 | 验证方法 | 量化阈值 | 章节 |
|---|---------|--------|-----------|---------|---------|------|
| T-1 | 工具描述五要素 | P0 | 准确率仅 55% | 逐个工具检查 description | 覆盖率 100% | Ch5 KP 5.6.1 |
| T-2 | 结构化错误协议 | P0 | 自愈率仅 32% | 触发错误验证 JSON 返回 | `{errorCode,suggestion,retryable}` | Ch5 KP 5.5.4 |
| T-3 | 工具数 >25 时 Semantic Router | P1 | 50+ 工具准确率 0-20% | 压测工具选择准确率 | ≥ 92% | Ch5 KP 5.4.2 |
| T-4 | 写操作幂等键 | P1 | 重复扣款/删除 | 模拟重放写操作 | 重复执行 = 1 次 | Ch5 KP 5.5.1 |
| T-5 | MCP Server 在沙箱中运行 | P1 | MCP 被劫持 | 检查部署位置 | 100% 在沙箱内 | Ch5 KP 5.2.2 |
| T-6 | MCP 工具描述 SHA-256 pinning | P1 | 工具描述被篡改 | 对比 pin 值 | pin 一致率 100% | Ch5 KP 5.2.2 |
| T-7 | 工具版本管理（新版迁移+旧版 30 天退役） | P2 | 旧版本僵尸调用 | 检查版本管理配置 | 退役窗口 30 天 | Ch5 KP 5.7.2 |
| T-8 | 协议选型有书面决策（FC/MCP/A2A） | P2 | 选型不当导致重构 | 检查决策文档 | 有书面记录 | Ch5 §5.3 |

### C.4.3 C 层 · 上下文与记忆

| # | 检查内容 | 优先级 | 不做的代价 | 验证方法 | 量化阈值 | 章节 |
|---|---------|--------|-----------|---------|---------|------|
| C-1 | system prompt 不含动态内容 | P1 | KV-cache 命中率 0，成本翻倍 | grep system prompt 中的时间戳/sessionId | 动态内容 = 0 | Ch6 §6.3 |
| C-2 | 上下文压缩机制 | P1 | 50 轮后 token 溢出 | 模拟长对话验证压缩触发 | 触发阈值 + 压缩策略 | Ch6 KP 4.4.1 |
| C-3 | KV-cache 命中率监控 | P1 | 成本失控无感知 | 查看监控指标 | 命中率 ≥ 50% | Ch6 §6.3 |
| C-4 | 三层记忆架构 | P2 | 跨会话失忆 | 检查记忆层配置 | 工作+情景+语义 | Ch6 §6.2 |
| C-5 | 关键约束每 10 轮重复注入 | P2 | 上下文腐烂，早期信息丢失 | 检查注入逻辑 | 间隔 ≤ 10 轮 | Ch6 KP 4.4.1 |
| C-6 | 上下文腐烂诊断 probe | P2 | 腐烂无感知无法触发压缩 | 每 N 步验证早期信息召回 | 召回率 ≥ 80% | Ch6 KP 4.4.1 |
| C-7 | 目标锚定 + 漂移检测 | P2 | Agent 偏离原始目标 | embedding cosine similarity 监控 | cosine < 0.6 告警 | Ch6 KP 4.4.2 |

### C.4.4 L 层 · 生命周期与编排

| # | 检查内容 | 优先级 | 不做的代价 | 验证方法 | 量化阈值 | 章节 |
|---|---------|--------|-----------|---------|---------|------|
| L-1 | 步数硬上限 | P0 | $47K 循环事故重演 | 配置检查 + 压测 | ≤ 30 步 | Ch7 KP 5.1.2 |
| L-2 | 连续重复检测（N=3 跳出） | P0 | 在空结果上死循环 | 模拟连续重复 | 3 次后强制跳出 | Ch7 KP 5.1.2 |
| L-3 | per-session 成本上限 + 熔断 | P0 | 单用户拖垮组织预算 | 模拟超限 | 自动终止 | Ch17 KP 4.3.1 |
| L-4 | 多层停止机制（L+V+O 三层） | P1 | 单层失效无兜底 | 检查三层停止配置 | 三层独立 | Ch7 KP 5.1.2 |
| L-5 | 检查点/恢复机制 | P1 | 崩溃后从头开始 | 杀进程验证恢复 | 可从 checkpoint 续跑 | Ch7 KP 5.5.2 |
| L-6 | 编排模式选型有书面记录 | P1 | 默认 ReAct 成本失控 | 检查决策文档 | 有选型记录 | Ch7 §7.6 |
| L-7 | 多 Agent 独立沙箱 + 独立步数 | P1 | 子 Agent 失控拖垮父 Agent | 检查多 Agent 配置 | 独立隔离 | Ch15 §15.4 |
| L-8 | Agent 拆分遵循拆分判据矩阵 | P2 | 过度拆分 O(N²) 通信开销 | 检查拆分文档 | 符合判据 | Ch15 §15.1 |

### C.4.5 O 层 · 可观测性

| # | 检查内容 | 优先级 | 不做的代价 | 验证方法 | 量化阈值 | 章节 |
|---|---------|--------|-----------|---------|---------|------|
| O-1 | 五标签成本归因 | P0 | 月底账单看不清 | 查询近 24h 成本按标签聚合 | 五标签全覆盖 | Ch8 KP 4.4.1 |
| O-2 | JSONL 事件日志 + traceId | P0 | 出事无法回放推理过程 | 抽查一条 trace 完整还原 | 100% 调用有 traceId | Ch8 KP 4.5.1 |
| O-3 | P99 成本异常检测 + 告警 | P1 | 成本异常月底才发现 | 注入异常验证告警 | 1h 内触发 | Ch8 KP 4.4.2 |
| O-4 | 三层分级存储（热/温/冷） | P1 | 日志存储成本失控 | 检查存储策略 | 热 7d/温 30d/冷归档 | Ch8 §8.6 |
| O-5 | P0 观测点 100% 采集 | P1 | 关键推理轨迹丢失 | 检查采集配置 | 模型/工具/入站/离站 100% | Ch8 §8.3 |
| O-6 | 每个 Middleware 的假设声明 + drift 检测 | P2 | 假设失效无感知 | 检查 Middleware 假设文档 | 覆盖率 ≥ 80% | Ch8 KP 4.2.2 |
| O-7 | Guides vs Sensors 二分框架 | P2 | 只有事后感知无前置引导 | 检查 Guides 配置 | 前置+后置双保险 | Ch8 KP 4.2.1 |

### C.4.6 V 层 · 验证与评估

| # | 检查内容 | 优先级 | 不做的代价 | 验证方法 | 量化阈值 | 章节 |
|---|---------|--------|-----------|---------|---------|------|
| V-1 | 100+ 任务回归评估集 | P0 | 改一处坏三处 | 检查评估集规模 | ≥ 100 任务 | Ch9 KP 5.1.2 |
| V-2 | CI 门禁：故障率 > 10% 阻断 | P0 | 带病上线 | 触发回归验证门禁 | 阻断生效 | Ch16 KP 4.1.1 |
| V-3 | 评估集按难度切分（简:中:难=3:5:2） | P1 | 评估偏差，难度分布失衡 | 检查评估集分布 | 3:5:2 ± 10% | Ch9 KP 5.4.2 |
| V-4 | LLM-as-Judge 三招偏差治理 + r > 0.8 | P1 | 评分不可靠 | 用标注集验证一致性 | Pearson r > 0.8 | Ch9 §9.3 |
| V-5 | A/B 测试 N ≥ 100 + p < 0.05 + d > 0.2 | P1 | 小样本误判 | 检查最近 A/B 报告 | 三条件全满足 | Ch9 §9.5 |
| V-6 | 影子流量验证（5% → 100%） | P2 | 全量部署风险高 | 检查灰度配置 | 渐进式扩流 | Ch9 KP 5.5.1 |
| V-7 | 自有评估集不与公开基准共享数据 | P2 | 基准污染，分数虚高 | 对比评估集与 SWE-bench | 数据无重叠 | Ch9 KP 5.3.2 |

### C.4.7 G 层 · 治理与安全

| # | 检查内容 | 优先级 | 不做的代价 | 验证方法 | 量化阈值 | 章节 |
|---|---------|--------|-----------|---------|---------|------|
| G-1 | 四检查点全覆盖（输入/工具/输出/会话） | P0 | 攻击从任意位置进入 | 检查 Middleware 链配置 | 4/4 检查点 | Ch10 KP 4.2.1 |
| G-2 | L1 正则过滤（OWASP Top 10） | P0 | 提示注入 | AgentDojo 100 条测试 | 拦截率 ≥ 95% | Ch10 KP 4.2.2 |
| G-3 | 危险操作白名单 + 人工审批 | P0 | `DROP TABLE`/`rm -rf` | grep 危险 @Tool | requiresApproval | Ch10 KP 4.2.2 |
| G-4 | PII 脱敏（正则 + 掩码） | P0 | 输出敏感信息 | 50 条 PII 测试 | 泄露率 = 0 | Ch10 KP 4.2.1 |
| G-5 | L2 分类器检测（<50ms） | P1 | 漏掉变体注入 | AgentDojo 高级用例 | 拦截率 ≥ 85% | Ch10 KP 4.2.2 |
| G-6 | YAML 声明式策略 + Git 版控 | P1 | 策略散落不可审计 | 检查策略仓库 | Git 版控 | Ch10 KP 4.3.1 |
| G-7 | WORM 审计日志 + 决策链追溯 | P1 | 事故后无法归责 | 抽查决策追溯 | 6 层归因完整 | Ch10 §10.5 |
| G-8 | 治理有效性指标（P/R/FPR） | P2 | 误杀正常请求或漏放攻击 | 查看治理指标看板 | P>85%/R>90%/FPR<5% | Ch10 §10.4 |
| G-9 | 新规则影子模式运行 1-2 周 | P2 | 规则误杀线上流量 | 检查规则上线流程 | 影子 1-2 周 | Ch10 §10.4 |
...
| 安全拦截误杀率 | < 5% | > 10% → P2 告警 | 滚动 24h | Ch10 §10.4 |
...
| 安全事件 | AgentDojo 类注入成功 | ① 立即下线 ② 审计日志追溯 ③ 补 G 层规则 | 规则上线 + 影子模式验证通过 | Ch10 §10.5 |
...
- **AgentScope core 内置**：Sandbox、DockerSandboxClient
- **agentscope-harness 扩展**：SandboxExecutionGuard、SandboxLifecycleMiddleware、FileSystemPolicy、CompositeFilesystem、OverlayFilesystem
- **CodePilot 示例组件**：SandboxManager（对 Sandbox 的进一步封装示例）
- **反模式**：生产环境中不启用沙箱、沙箱与宿主机共享文件系统

### T 层 · 工具接口与协议
- **核心概念**：工具 = 描述（Description）+ 参数（Parameters）+ 执行逻辑（Logic）+ 错误协议（Error Protocol）
- **设计决策**：粒度（黄金粒度原则）、协议选择（Function Calling / MCP / A2A）、语义路由（Top-K 预选）
- **AgentScope core 内置**：@Tool、@ToolParam、Toolkit、McpClientWrapper/McpClientBuilder（MCP 集成）
- **agentscope-harness 扩展**：SkillCuratorMiddleware、SkillSecurityScanner（技能策展与安全扫描）、AsyncToolMiddleware、ToolResultEvictionMiddleware
- **CodePilot 示例组件**：ToolCallingMiddleware
...
- **AgentScope core 内置**：Memory、InMemoryMemory、StateBackedMemory
- **agentscope-harness 扩展**：CompactionMiddleware、MemoryFlushMiddleware、MemoryMaintenanceMiddleware、MemoryConsolidator
- **CodePilot 示例组件**：MessageChatMemoryMiddleware、VectorStoreChatMemoryMiddleware
...
- **AgentScope core 内置**：ReActAgent（核心 Agent）
- **agentscope-harness 扩展**：PlanModeMiddleware
- **CodePilot 示例组件**：ReActLoopMiddleware、PipelineOrchestrator、SupervisorAgent
...
- **AgentScope core 内置**：OtelTracingMiddleware、Tracer、TracerRegistry（追踪门面）
- **agentscope-harness 扩展**：AgentTraceMiddleware
- **CodePilot 示例组件**：CostAttributionMiddleware、EventLogRecorder
- **反模式**：观测事后加导致关键推理轨迹丢失、成本异常到月底才发现

### V 层 · 验证与评估
- **核心概念**：五阶段质量控制循环、LLM-as-Judge（三招治偏差）、自有评估集 + 影子流量
- **设计决策**：锚定标准、嵌入式评判 vs 终点评判、A/B 样本量（N ≥ 100 + Cohen's d > 0.2）
- **AgentScope core 内置**：无（V 层组件全部由扩展层和示例层提供）
- **agentscope-harness 扩展**：SessionFreshnessEvaluator（会话新鲜度评估）
- **CodePilot 示例组件**：ReadinessCheckMiddleware、EmbeddedValidationMiddleware、RegressionEvaluator
...
- **AgentScope core 内置**：PermissionEngine、PermissionRule、PermissionMode（权限引擎）
- **agentscope-harness 扩展**：SandboxExecutionGuard、PathPolicy、ShellPathPolicy、SkillSecurityScanner、SkillAuditLog、AdminAuditLogger
- **CodePilot 示例组件**：SafeGuardMiddleware、InputGuardMiddleware、ToolGuardMiddleware、PolicyLoader
...
ReActAgent agent = ReActAgent.builder(model)
    ...
    .toolkit(toolkit)
    .middlewares(List.of(
        new SafeGuardMiddleware(guardConfig),  // CodePilot 示例组件（非 AgentScope 内置）
        new MessageChatMemoryMiddleware(memory), // CodePilot 示例组件
        new SimpleLoggerMiddleware(),           // CodePilot 示例组件
        new OtelTracingMiddleware()             // AgentScope 真实内置
    ))
    .maxIters(10)
    .build();
...
// ========== OtelTracingMiddleware：可观测 ==========
// AgentScope 内置 OpenTelemetry 追踪中间件，注册即可启用
ReActAgent agent = ReActAgent.builder(model)
...
> CodePilot 配套仓库已发布：`codepilot/` 目录，16 个 Maven 模块，67 个 Java 文件，可通过 `mvn clean install -DskipTests` 编译，`codepilot-starter` 模块统一启动。代码可用性详见 [COVERAGE_MAP.md](../codepilot/COVERAGE_MAP.md)。

| 章节 | 模块名 | 核心文件 | 代码状态 | 测试数 | 说明 |
|------|--------|---------|---------|--------|------|
| Ch1 | `ch01-foundation` | `SimpleAgentExample.java`、`PdaLoopService.java`、`AgentLoopFailureModes.java` | 🟡 Runnable | 0 | 7 个 Java 文件（~460 行）可运行，无单元测试 |
| Ch2 | `ch02-definition` | — | 🔴 Skeleton | 0 | 结构占位，不可运行 |
| Ch3 | `ch03-behavior` | `RetryOrchestrationMiddleware.java` | 🟡 Runnable | 44 | 3 个测试文件，核心路径有覆盖 |
| Ch4 | `ch04-sandbox` | `SandboxExecutionEnvironment.java` | ✅ Tested | 17 | SandboxPoolTest 覆盖核心逻辑 |
| Ch5 | `ch05-tools` | `SemanticToolRouter.java`、`ToolValidationMiddleware.java` | 🟡 Runnable | 28 | 3 个测试文件 |
| Ch6 | `ch06-memory` | `MemoryManager.java`、`ContextBudgetMiddleware.java` | ✅ Tested | 75 | KnowledgeBaseTest(62)+WorkingMemory(13) |
| Ch7 | `ch07-orchestration` | `ReActOrchestrator.java`、`PipeReActMiddleware.java` | ✅ Tested | 36 | ReActOrchestratorTest(29)+LoopDetection(7) |
| Ch8 | `ch08-observability` | `CostTracker.java`、`BurnRateCalculator.java` | 🟡 Runnable | 19 | CostTracker(13)+AssumptionDrift(6) |
| Ch9 | `ch09-evaluation` | `RegressionRunner.java`、`LLMJudgeMiddleware.java` | 🟡 Runnable | 33 | Evaluation(17)+ReadinessCheck(16) |
| Ch10 | `ch10-governance` | `InputGuardMiddleware.java`、`ConstitutionValidator.java` | 🟡 Runnable | 18 | InputGuard(7)+AuditLog(11) |
| Ch12 | `ch12-model` | `ModelRouter.java`、`ModelFallbackService.java`、`ModelEnsemble.java` | 🟡 Runnable | 0 | 5 个 Java 文件（~282 行）可运行，无单元测试 |
| Ch13 | `ch13-data` | `ThreeLayerKnowledgeRouter.java`、`RagRetriever.java` | 🔴 Skeleton | 0 | 结构占位，不可运行 |
| Ch14 | `ch14-planning` | `TaskPlanner.java`、`ReActReasoner.java` | 🔴 Skeleton | 0 | 结构占位，不可运行 |
| Ch15 | `ch15-multiagent` | `DagDecomposer.java`、`ConsistencyVerifier.java`、`WorkerCircuitBreaker.java`、`HintBlockMessageBus.java`、`GovernedMemoryStore.java` | 🟡 编译通过（无测试） | 37 | —（DagDecomposerTest 已删除：引用不存在的 API，从未编译通过） |
| Ch16 | `ch16-mlops` | `EvalPipeline.java`、`CanaryDeployer.java` | ✅ Tested | 14 | GatePipelineOrchestratorTest |
| Ch17 | `ch17-reliability` | `SlaMonitor.java`、`CostAttribution.java` | ✅ Tested | 27 | FourLayerBudgetEnforcerTest |
| Full | `codepilot-starter` | `CodepilotApplication.java` | ✅ Runnable | — | 统一启动入口 |
| Core | `codepilot-core` | `Layer.java` | ✅ Runnable | — | 七层枚举定义 |

**代码状态图例**：✅ 可运行+有测试 | 🟡 可运行（测试覆盖不全或缺失） | 🔴 骨架（仅结构占位，不可运行）
...
|------|-------------------|-------------------|-----------------|
| 沙箱隔离 | 自定义 `SandboxManager` + Docker SDK | `langchain.tools` + 自定义 subprocess | 框架不提供，需自建 |
| 超时控制 | `@Tool(timeout=...)` 注解参数 | `tool.run(timeout=...)` | `Task(timeout=...)` |
| 资源限制 | 自定义 `ResourceQuotaManager` | 通过 Docker SDK 设置 | 不支持 |
| 快照/回滚 | 自定义 `CheckpointManager` + OverlayFS | `langgraph.checkpoint` | 不支持 |
...
| MCP 集成 | `McpClient` | `langchain-mcp-adapters` | 不支持 |
...
| 语义记忆 | 自定义 `CodeBaseIndex` | LangChain 内置 `VectorStore` + `Document` | 不支持 |
| 上下文压缩 | `CompactionMiddleware` (agentscope-harness 提供) | `summarizationMiddleware` | 不支持 |
| KV-cache | 自定义 `KV-CacheTracker` (建议) | Anthropic API 原生支持 `cache_control` | 不支持 |
...
| 熔断降级 | 需自定义（CodePilot 示例：`CircuitBreakerMiddleware`） | 需自定义 Callback | 不支持 |
...
| 概念 | AgentScope | LangChain | CrewAI |
|------|-----------|-----------|--------|
| Traces | `OtelTracingMiddleware` → OTLP（AgentScope 内置） | LangSmith / LangFuse | 仅控制台输出 |
| Metrics | `OtelTracingMiddleware` → Prometheus（配合 Micrometer） | LangSmith Dashboard | 不支持 |
...
**翻译要点**：LangChain 生态的可观测性高度依赖 LangSmith（商业产品）和 LangFuse（开源替代）。如果你不用 LangSmith，O 层需要自行搭建——AgentScope 的 `OtelTracingMiddleware` 内置了 OpenTelemetry 追踪，结合 Micrometer 可实现自控的可观测性管线。

### V 层 · 验证与评估

| 概念 | AgentScope | LangChain | CrewAI |
|------|-----------|-----------|--------|
| LLM-as-Judge | 自定义 `LLMJudgeEvaluator` | LangChain `StringEvaluator` / `QAEvaluator` | 不支持 |
...
| YAML 策略管理 | 自定义 `PolicyLoader` | 需自定义 | 需自定义 |
| 审计日志 | 自定义 WORM 存储 | LangSmith 审计 | 不支持 |


## C.11 Agent 框架选择决策树

> 用三个问题决定你的 Agent 技术栈，避免成为"框架选择不当是项目失败第一原因"（Ch0 [^13]）的统计数据。
...
并不是说不能选它——是说你需要在选型时就清楚这些空白格存在，并在项目计划中预留填补时间。参考 Ch0 框架覆盖表、C.10 节的 AgentScope ↔ LangChain 映射、以及第 16 章的阶段化投入指南，建立一个"填补空白格"的工作计划。
...
| CompositeFilesystem | Composite Filesystem | 组合式文件系统抽象——AgentScope-Java 用于组合宿主机受限目录与沙箱临时层的文件系统架构 |
...
| 五区制预算 | Five-Zone Budgeting | 按预算消耗比例划分五个区域（正常/关注/警告/严重/熔断），每区有独立的响应策略 |
| CodePilot | CodePilot | 本书贯穿案例——从裸模型 42% 成功率通过七层 Harness 逐章叠加达到 82% 的编程 Agent |
| AgentScope-Java | AgentScope-Java | 分布式 Agent 沙箱框架——为 E 层提供跨 Docker/gVisor/Firecracker 的统一执行环境抽象 |
| SandboxClient | Sandbox Client | AgentScope-Java 中的沙箱客户端——Agent 通过此客户端在隔离环境中执行代码 |
| WorkspaceManager | Workspace Manager | AgentScope-Java 中的工作区管理器——负责沙箱文件系统的挂载和生命周期管理 |
| SkillCurator | Skill Curator | AgentScope-Java 中的技能策展器——负责工具/技能的发现、验证和注册管理 |
| 五要素描述框架 | Five-Element Description Framework | 工具描述的五要素规范——名称、目的、参数、返回值、副作用——T 层工具注册的核心设计规范 |
...
| τ-bench | — | 多步工具使用任务 | Ch0, Ch2 | 工具型 Agent 可靠性 |
