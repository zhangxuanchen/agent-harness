...
- 4.3 五种沙箱方案怎么选：从隔离强度到 TCO 的完整决策
  ...
  - KP 4.3.7 只看云账单会踩坑：TCO 模型的隐性成本【构建】
...
- 4.5 沙箱跑起来后怎么管：冷启动、复用与安全防御
  ...
  - KP 4.5.4 Agent 不恶意也会耗尽资源：八维硬限制【诊断】
...
- 5.1 工具：Agent 的"手"
  - KP 5.1.1 工具的本质：把能力封装为模型可理解、可调用的接口【构建】
  - KP 5.1.2 工具的粒度假说：太大不灵活，太小组合爆炸【诊断】
- 5.2 工具协议的演进：Function Calling → MCP → A2A
  - KP 5.2.1 Function Calling：模型原生的工具调用能力【构建】
  - KP 5.2.2 MCP（Model Context Protocol）：标准化的工具暴露协议【构建】
  - KP 5.2.3 A2A（Agent-to-Agent）：Agent 间通信协议【构建】
- 5.3 MCP vs A2A：互补还是竞争
  - KP 5.3.1 互补场景：MCP 提供外部能力，A2A 协调内部协作【构建】
  - KP 5.3.2 何时不需要两者：单 Agent 系统的简化选择【构建】
- 5.4 工具发现、选择与组合编排
  - KP 5.4.1 工具发现：从静态注册到动态发现【构建】
  - KP 5.4.2 工具选择：模型自主选 vs 路由预选 vs 混合【构建】
  - KP 5.4.3 工具组合：序列化依赖与并行调用的优化【构建】
- 5.5 工具设计最佳实践：幂等、可观测、可回滚、错误可恢复
  - KP 5.5.1 幂等性：相同输入相同输出，重复调用安全【诊断】
  - KP 5.5.2 可观测：每次工具调用留下完整的审计线索【构建】
  - KP 5.5.3 可回滚：写操作的前置状态保存【构建】
  - KP 5.5.4 错误可恢复：返回结构化错误而非崩溃【诊断】
- 5.6 工具描述工程：让模型"看懂"工具
  - KP 5.6.1 好的工具描述应该包含什么【构建】
  - KP 5.6.2 描述工程的反模式：模糊、歧义、隐式约束【诊断】
- 5.7 工具运行时治理与动态生命周期
  - KP 5.7.1 工具的运行时权限管理：白名单、频率限制、配额【构建】
  - KP 5.7.2 工具的版本管理与退役【构建】

### 第 06 章 — C：上下文与记忆

- 6.1 上下文窗口：稀缺且昂贵的资源
  - KP 6.1.1 输入 token 为什么占总费用的 70-85%：上下文的成本结构与预算控制【构建】
  - KP 6.1.2 为什么窗口大了模型还是看不到：注意力衰减效应【诊断】
- 6.2 上下文治理四动作：Write / Select / Compress / Isolate
  - KP 6.2.1 上下文治理的本质是什么：控制信号密度【构建】
  - KP 6.2.2 模型读窗口为什么不是均匀的：Lost-in-the-Middle 位置偏差【诊断】
  - KP 6.2.3 工具输出怎么结构化：Write 动作的源头治理【构建】
  - KP 6.2.4 怎么知道治理得好不好：组装审计的反馈闭环【构建】
- 6.3 Write 动作：什么能进上下文，什么不能，三层记忆写入
  - KP 6.3.1 工作记忆：如何管当前任务的状态与轨迹【构建】
  - KP 6.3.2 情景记忆：如何跨会话保留历次经历【构建】
  - KP 6.3.3 语义记忆：如何从大量经历中蒸馏通用知识【构建】
- 6.4 Compress 动作（一）：KV-cache 稳定前缀设计
  - KP 6.4.1 KV-cache 怎么工作：前缀复用为什么能省 30-50% 成本【构建】
  - KP 6.4.2 什么操作会影响缓存命中率：缓存击穿诊断【诊断】
- 6.5 Isolate 动作（一）：上下文腐烂与漂移两种静默故障
  - KP 6.5.1 信息还在窗口里为什么模型用不上：上下文腐烂【诊断】
  - KP 6.5.2 Agent 为什么会跑偏：上下文漂移【诊断】
- 6.6 Compress（二）+ Select：压缩怎么做 + 记忆怎么挑
  - KP 6.6.1 摘要怎么做才不丢信息：提取式、生成式与混合策略【构建】
  - KP 6.6.2 RAG 在上下文工程中扮演什么角色：检索增强的辅助压缩【构建】
- 6.7 多 Agent 上下文治理：交接、共享与安全边界
  - KP 6.7.1 多 Agent 协作时上下文怎么传递：Handoff 与共享记忆【构建】
  - KP 6.7.2 外部内容进上下文怎么防注入：上下文安全面【诊断】
  - KP 6.7.3 会话恢复时怎么判断历史上下文是否还有效：Session Resume 过期检测【诊断】
- 6.8 怎么知道四动作治理得好不好：记忆系统评测闭环
  - KP 6.8.1 检索到的记忆是不是真的有用：准确率与召回率评测【构建】
  - KP 6.8.2 压缩后的记忆丢了多少：信息保真度评测【构建】
...
- 7.4 完整管道案例：Issue → PR 端到端
  - KP 7.4.1 管道的五阶段分解与每阶段的风险点【构建】
  - KP 7.4.2 管道的端到端成功率优化：从 40% 到 90%【构建】
- 7.5 状态机与恢复：让 Agent 能"断点续传"
  - KP 7.5.1 Agent 状态机的设计：状态、事件、转换【构建】
  - KP 7.5.2 检查点的设计：存什么、存多频【构建】
- 7.6 编排模式性能基准对比
  - KP 7.6.1 四种编排模式在标准任务上的横向基准【构建】
  - KP 7.6.2 模式选择的决策树【构建】
- 7.7 混合编排架构
  - KP 7.7.1 PipeReAct：Pipeline 骨架 + ReAct 肌肉【构建】
  - KP 7.7.2 Plan+Execute+Verify：三层嵌套模式【构建】
...
- 9.1 为什么 Agent 评估特别难
  - KP 9.1.1 Agent 评估的四重困难【诊断】
  - KP 9.1.2 评估驱动的 Agent 开发循环【构建】
- 9.2 四阶段质量控制循环：锚定 → 就绪 → 嵌入式评判 → 回归
  - KP 9.2.1 锚定（Anchor）：定义"好"的标准【构建】
  - KP 9.2.2 就绪（Readiness）：评估前的输入检查【构建】
  - KP 9.2.3 嵌入式评判：执行过程中的实时检查【构建】
  - KP 9.2.4 回归（Regression）：持续的质量守护【构建】
- 9.3 评估方法：人工 / 自动 / LLM-as-Judge 及其偏差
  - KP 9.3.1 LLM-as-Judge：优势、局限与偏差治理【诊断】
  - KP 9.3.2 "度量陷阱"：基准分高 ≠ 用着好【诊断】
- 9.4 评估数据集与基准
  - KP 9.4.1 主流基准的能力边界与适用性【构建】
  - KP 9.4.2 构建自有评估集的方法论【构建】
- 9.5 在线评估、A/B 与影子流量
  - KP 9.5.1 影子流量评估：不打扰用户的真实环境测试【构建】
  - KP 9.5.2 A/B 测试的统计陷阱【诊断】
- 9.6 评估系统的元评估
  - KP 9.6.1 Judge 漂移检测：评判者自身的质量监控【诊断】
  - KP 9.6.2 评估覆盖率分析：发现未覆盖的业务场景【构建】
